	...
	function combinar_csv($dependencia,$codusuario,$usua_doc,$usua_nomb,$depe_codi_territorial,$codiTRD,$tipoRad){
		...
		if ($fp){
			...
			for($ii=0; $ii < count ($this->datos) ; $ii++){   
                ...
                $nombre_us      = utf8_decode($nombre_us);
                $direccion_us1  = utf8_decode($direccion_us1);
                $dpto_nombre    = utf8_decode($dpto_nombre);
                $muni_nombre    = utf8_decode($muni_nombre);

                $nombre_us2     = utf8_decode($nombre_us2);
                $direccion_us12 = utf8_decode($direccion_us12);
                $dpto_nombre2    = utf8_decode($dpto_nombre2);
                $muni_nombre2    = utf8_decode($muni_nombre2);
