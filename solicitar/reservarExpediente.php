              ...
              <div class="row" >
                ...
                <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                  ...
                  <div>
                      ...
                      Reserva de documentos del Expediente No. <?=$_SESSION['dataPresExpediente']['expediente']?></span>
                      </h1>
                  </div>
...
function PRESTAMO_action($sAction) {
    ...
    if ($sAction=="cancelar") {
        ...
        // Se mantiene la funcion get_param().
        //$valSolPrest = $_POST["s_PRES_REQUERIMIENTO"];
        $valSolPrest = get_param("s_PRES_REQUERIMIENTO");
        $fldPRES_FECH_PEDI = $db->conn->OffsetDate(0,$db->conn->sysTimeStamp);
        // Obtiene la ubicacion fisica de los documentos
        $fldPRES_DEPE_ARCH=substr($fldradicado,4,3);
        // $fldPRES_DEPE_ARCH=;
...
<?
         ...
         while($rs && !$rs->EOF) {
             ...
             $valSolPrest=$rs->fields["REQUERIMIENTO"];
             ...
             // HTML prestamo show begin	  ?>
                        <tr <? echo $tipoListado; ?> align="center">
                           ...
                           <td class="leidos"><?= tohtml($valSolPrest); ?></td>
      ...
      if (strlen($sqlReq)!=0) {
        $sqlReq = "SELECT
                   ...
                   FROM
                     SGD_PARAMETRO
                   WHERE
                     PARAM_NOMB='PRESTAMO_REQUERIMIENTO' and
                     PARAM_CODI".$sqlReq."
                     order by PARAM_VALOR ";
?>
               <script>
