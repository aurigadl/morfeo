 ...
 function insertarTRD($codiTRDS,$codiTRD,$noRadicado, $coddepe , $codusuario, $tdoc = null){		
    ...
    if($rs->RowCount()>=1){
        ...
        foreach($codiTRDS as $codiTRDR)
        {
            ...
            $tiemTdoc = $rs2->fields["SGD_TPR_TERMINO"];
            ...
            //date("j-n-Y",strtotime("2000-10-29 + 3 year 1 month 2 days"))
            $emp1 = $tiemTdoc = empty($tiemTdoc)? 30 : $tiemTdoc;

            //Bucamos los dias habiles
            //convertir a hora unix
            $fechini      = time();
            $frad         = substr($fechRad,0,10);
            $fechradicado = strtotime($frad);
            $conhabi      = $contado = 0;
            
            $fechfin1     = strtotime("$frad + $tiemTdoc day");
            $noHabil      = getdate($fechfin1);
            $diasuma      = $noHabil["wday"];
            
            while(($diasuma == 0) or ($diasuma == 6) or in_array($fechfin1,$festi)){
                $contado++; 
                $tmp      = $tiemTdoc + $contado;
                $fechfin1 = strtotime("$frad + $tmp day");
                $noHabil  = getdate($fechfin1);
                $diasuma  = $noHabil["wday"];
            }
            ...
            while($fechradicado < $fechfin1){

                $noHabil  = getdate($fechfin1);

                if (($noHabil["wday"] == 0) or ($noHabil["wday"] == 6) or in_array($fechfin1,$festi)){
                    $conhabi++;
                }
                //si la fecha es festiva agregelo a los no habiles
                $fechfin1 = strtotime(date("Y-m-d",$fechfin1).' - 1 day'); //Formatea dia
            }

            //fecha final del resultado
            $tmp    = date("Y-m-d",$fechfin2);
            $diasRe = $conhabi - $contado; 
            $resul  = date("Y-m-d",strtotime("$tmp + $diasRe day"));  }
            /*echo " <br> fecha radicado $frad
              <br> fecha final sin procesar $tmp
              <br> dias por tipo documental $emp1
              <br> fecha procesando $resul";*/

			$record = array(); # Inicializa el arreglo que contiene los datos a modificar
			//(date('',$resul)) 
		        if(date("D", strtotime($resul))=="Sat") $resul = date("Y-m-d", strtotime("$resul + 2 day"));
			if(date("D", strtotime($resul))=="Sun") $resul = date("Y-m-d", strtotime("$resul + 1 day"));
